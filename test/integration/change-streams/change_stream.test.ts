...
describe('Change Streams', function () {
  ...
  context('multiple change streams', () => {
    let changeStream1: ChangeStream;
    let changeStream2: ChangeStream;
    let changeStream3: ChangeStream;
    let client;
    beforeEach(async function () {
      client = this.configuration.newClient();
    });
    ...
    it('should support creating multiple simultaneous ChangeStreams', {
      ...
      test: async function () {
        ...
        const laterInsert = sleep(50).then(async () => {
          await collection1.insertMany([{ a: 1 }]);
          await collection2.insertMany([{ a: 1 }]);
        });
        ...
        expect(changes[0]).to.have.property('operationType', 'insert');
        expect(changes[1]).to.have.property('operationType', 'insert');
        expect(changes[2]).to.have.property('operationType', 'insert');
