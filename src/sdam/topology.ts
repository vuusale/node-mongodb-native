...
export class Topology extends TypedEventEmitter<TopologyEvents> {
  ...
  connect(options?: ConnectOptions | Callback, callback?: Callback): void {
    ...
    const ctx = Context.fromOptions(this.client, options);
    this.selectServer(
      readPreferenceServerSelector(readPreference),
      { ctx, operationName: 'ping' },
      (err, server) => {
        if (err) {
        ...
        if (!skipPingOnConnect && server && this.s.credentials) {
          server.command(ns('admin.$cmd'), { ping: 1 }, ctx, err => {
            if (err) {
  ...
  selectServer(
    ...
    const waitQueueMember: ServerSelectionRequest = {
      ...
      ctx: options.ctx,
      timeout: options.ctx.timeouts.serverSelectionTimeout()
    };
